...
export default function SkillsSection() {
  ...
  useEffect(() => {
   ...
   const parallaxMultiplier = isMobile ? 10 : 30;
   
  return (
    <section ref={sectionRef} className="snap-start min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center py-12 sm:py-16 px-4 sm:px-6 overflow-hidden">
      <div className="max-w-7xl mx-auto w-full">

        <h2 className="text-center text-3xl sm:text-4xl md:text-5xl font-bold mb-8 sm:mb-12 md:mb-16 text-slate-900 dark:text-white">
          Technical Skills
        </h2>

        {/* Single combined grid for all skills */}
        <div 
          className="grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4 md:gap-6 lg:gap-8"
          style={{
            transform: `translateX(${scrollY * parallaxMultiplier - parallaxMultiplier / 2}px)`
          }}
        >
          ...
          {bottomRow.map((skill, index) => (
            <SkillCard key={skill.name} icon={skill.icon} name={skill.name} index={index + topRow.length} />
          ))}
        </div>
...
function SkillCard({ icon, name, index }: { icon: string; name: string; index: number }) {
  ...
  const svgs: Record<string, ReactElement> = {
    ...
    cpp: (
      <svg viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg" className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 mb-3 sm:mb-4">
  ...
  return (
    <div 
      className="flex flex-col items-center justify-center bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 md:p-8 shadow-md hover:shadow-2xl transition-all duration-300 cursor-pointer group"
      style={{
    ...
    >
      ...
      <p className="text-slate-900 dark:text-white font-semibold text-sm sm:text-base md:text-lg text-center transition-colors group-hover:text-blue-600 dark:group-hover:text-blue-400">
        {name}
      </p>
    </div>
  );
